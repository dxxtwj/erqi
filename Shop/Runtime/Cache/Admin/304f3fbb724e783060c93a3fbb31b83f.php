...
<div class="left_add" style="width: 1500px">
   <div class="title_name" style="width: inherit;">修改品牌</div>
   <form action="<?php echo U('Brand/edit');?>" method='post' enctype="multipart/form-data">
   <input type="hidden" name='id' value="<?php echo ($arr['id']); ?>" />

   <ul class="add_conent">
   <!-- name="pinpainame" 是因为用了字段映射 -->
    <li class=" clearfix"><label class="label_name"><i>*</i>品牌名称：</label> <input name="name" value="<?php echo ($arr['name']); ?>" type="text" class="add_text"/></li>
    <li class=" clearfix"><label class="label_name">品牌图片：</label>
			<div style='display:inline;' onclick="up(this)" class="demo l_f"><span style="display: inline-block; " class="btn btn-xs btn-info"><i class="icon-edit bigger-120"></i>点我上传logo</span>
         ...
         <select name="region" id="">
			...
			<option <?php echo ($arr['region'] == 1 ? selected : ''); ?> value="1">国内</option>
			<option <?php echo ($arr['region'] == 2 ? selected : ''); ?> value="2">国外</option>
         </select>
         </li>
         <li class=" clearfix"><label class="label_name">品牌描述：</label> <textarea name="content" cols="" rows="" class="textarea" onkeyup="checkLength(this);"><?php echo ($arr['content']); ?></textarea><span class="wordage">剩余字数：<span id="sy" style="color:Red;">500</span>字</span></li>
		...
		排序的位置：　
         ...
         <option value="">--请选择--</option>
			<option <?php echo ($arr['order'] == 1 ? selected : ''); ?> value="1">1</option>
			<option <?php echo ($arr['order'] == 2 ? selected : ''); ?> value="2">2</option>
			<option <?php echo ($arr['order'] == 3 ? selected : ''); ?> value="3">3</option>
			<option <?php echo ($arr['order'] == 4 ? selected : ''); ?> value="4">4</option>
			<option <?php echo ($arr['order'] == 5 ? selected : ''); ?> value="5">5</option>
			<option <?php echo ($arr['order'] == 6 ? selected : ''); ?> value="6">6</option>
			<option <?php echo ($arr['order'] == 7 ? selected : ''); ?> value="7">7</option>
			<option <?php echo ($arr['order'] == 8 ? selected : ''); ?> value="8">8</option>
			<option <?php echo ($arr['order'] == 9 ? selected : ''); ?> value="9">9</option>
			<option <?php echo ($arr['order'] == 10 ? selected : ''); ?> value="10">10</option>
         </select>
   </ul>
 </div>

   
  <div class="button_brand" style='margin-left: -40px;'><button class="btn btn-warning" >提交</button>
  <span class="btn btn-warning" /><a style='color: #000' href="<?php echo U('Brand/index');?>">返回</a></span></div>
		...
		$("#select1").change(function() {
			...
			$.ajax({
				...
				success: function(msg) {
					...
					$('#select2').append("<span style='width: 220px; display: block; text-align: center; bor: 1px solid #000; margin-top: 5px; value='" + msg.id + "''>"+ msg.name +"</span>");
				}
